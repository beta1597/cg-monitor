...
exit;
	...
	$sql_devs = "SELECT * FROM `cgmonitor__devs` where idrigs = '".$rig."' order by number;";
	$devices = result($sql_devs);
	
		foreach($devices as $device){
			$sql3 = "SELECT * FROM `miner_".$rig."_DEVS_".$device["number"]."` where Time > '".$timeyesterday."' order by Time";
			$result3 = result($sql3);
			...
			foreach($result3 as $result_det){
					...
					foreach($values as $value){
						...
						$temp_array[$stamp][$device["number"]]["value"][$value] 		= $temp;
						$temp_array[$stamp][$device["number"]]["value"]["hashwu"] 	= $hashwu;
					};
				};
		};
	...
	for ($i = $timeyesterday; $i <= $timenow; $i += 60) {
		...
		foreach($values as $value){
			...
			foreach($devices as $device){
				if(isset($temp_array[$timecheck][$device["number"]])){
					$temp = $temp_array[$timecheck][$device["number"]]["value"][$value];
					$WUs  = $temp_array[$timecheck][$device["number"]]["value"]["hashwu"];
				}else if(isset($temp_array[$timecheck2][$device["number"]])){
					$temp = $temp_array[$timecheck2][$device["number"]]["value"][$value];
					$WUs  = $temp_array[$timecheck2][$device["number"]]["value"]["hashwu"];
				}else if(isset($temp_array[$timecheck3][$device["number"]])){
					$temp = $temp_array[$timecheck3][$device["number"]]["value"][$value];
					$WUs  = $temp_array[$timecheck3][$device["number"]]["value"]["hashwu"];
				}else{
					$temp = 0;
					$WUs  = 0;
				};
				...
				$profit[$device["number"]][$value] 		.= $temp .",";
				$percent 						.= ",perc".$device["number"].":'".$WUs."'";
			};
		};
		$totalWUs = number_format(($totalWUs / $count_devices), 2, '.', '');
...
?>
					$.each(this.points, function(i, point) {
						if(this.x['perc' + point.series.name] != undefined){
							s += '<br/>'+ point.series.name +': '+ Math.round(point.y) +' KH/s (' + this.x['perc' + point.series.name] + '%)';;
							sum += point.y;
						};
					});
...
?>		
			{
                name: '<?php echo $device["number"];?>',
                data: [<?php echo $profit[$device["number"]][$value];?>],
				marker: {
                    enabled: false
                }
            },
