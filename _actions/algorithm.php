...
<?php
	$sql_sel = "SELECT * FROM `".$table."` where number = '".$id."' order by number ;";
	$res_sel = result($sql_sel);
	foreach($res_sel as $result_det){
?>
	<h1><?php echo urldecode($result_det["name"]);?> (<?php echo urldecode($result_det["number"]);?>)</h1>
...
<?php
	...
	foreach($res_pools as $pool){
		...
		echo "<td>&nbsp;&nbsp;&nbsp;<a href='?action=".$_GET['action']."&id=". $id ."&removepool=". $pool["poolid"] ."'><button type='button' class='btn btn-default btn-sm'><span class='glyphicon glyphicon-trash'></span></button></a></td>";
		echo "</tr>";
	...
	$sql_pools 	= "SELECT cgmonitor__pools.id as id,cgmonitor__pools.name as poolname, cgmonitor__algorithm.name as algoname
						...
						ON cgmonitor__algorithm.id =cgmonitor__pools.idalgorithms where cgmonitor__algorithm.id = '".$result_det["id"]."'
